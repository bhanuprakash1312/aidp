...
export default function Sidebar({ isOpen, closeSidebar }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`bg-gradient-to-b from-blue-50 to-blue-100 border-r border-blue-200 text-gray-800 shadow
  transform transition-transform duration-300 ease-in-out
  fixed md:static top-0 left-0 h-full md:w-64 z-40
  ...
  flex flex-col`}
      >
        ...
        {/* Menu links */}
        <div className="mt-6 space-y-2 flex-1 px-4">
          <Link
            to="/dashboard"
            onClick={closeSidebar}
            className={`flex items-centered gap-3 px-4 py-2 rounded-lg transition ${isActive(
              "/dashboard"
            )}`}
          >
        ...
        {/* Footer */}
        <div className="text-center text-xs text-blue-500 pb-4">
          v1.0 © 2025
        </div>
      </div>
    </>
